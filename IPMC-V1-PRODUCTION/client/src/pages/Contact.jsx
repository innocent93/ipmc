...
export default function Contact() {
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', company: '', subject: '', message: '', serviceInterest: '', website: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', phone: '', company: '', subject: '', message: '', serviceInterest: '', website: '' });
    } catch (error) {
            ...
            >
              {isSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-8 shadow-lg">
                  ...
                  {/* Honeypot: invisible to real users (off-screen, not
                      display:none — some bots skip hidden/display:none
                      fields specifically), tabIndex -1 so keyboard users
      ...
      <section className="pb-20 bg-white">
        <div className="container-custom">
          <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
            <iframe
              title="IPMC Head Office location — 18B Olu Holloway Road, Ikoyi-Lagos"
              src="https://www.google.com/maps?q=18B+Olu+Holloway+Road,+Ikoyi-Lagos,+Nigeria&output=embed"
              width="100%"
              height="420"
              style={{ border: 0 }}
              loading="lazy"
