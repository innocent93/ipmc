...
export default function NewsletterSubscribeForm({ compact = false, source = 'newsletter-page' }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.subscribe(normalized, source);
      setSuccess(true);
      setEmail('');
  ...
  return <form onSubmit={submit} className={`flex ${compact ? 'flex-col sm:flex-row' : 'flex-col sm:flex-row'} gap-3`}>
    ...
    <button type="submit" disabled={loading} className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-primary-700 text-white font-semibold hover:bg-primary-800 transition-colors disabled:opacity-60">{loading ? <Loader2 size={18} className="animate-spin" /> : 'Subscribe'}</button>
  </form>;
}
