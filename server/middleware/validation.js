...
exports.schemas = {
  ...
  contact: Joi.object({
    ...
    // Honeypot: a field real users never see or fill (hidden via CSS,
    // not `type="hidden"`, since some bots specifically skip those).
    // Bots that auto-fill every input trip this; humans never do.
  ...
  event: Joi.object({
    ...
    slug: Joi.string().min(3).max(200).required(),
    description: Joi.string().min(10).required(),
    category: Joi.string().valid('conference', 'workshop', 'seminar', 'training').optional(),
    coverImage: Joi.string().uri().allow('').optional(),
    location: Joi.string().required(),
  ...
  rsvp: Joi.object({
    ...
    email: Joi.string().email().required(),
    phone: Joi.string().allow('').optional(),
    guests: Joi.number().min(1).max(10).optional(),
  }),
  jobApplication: Joi.object({
    ...
    coverLetter: Joi.string().max(5000).allow('').optional(),
  }),
  settings: Joi.object({
