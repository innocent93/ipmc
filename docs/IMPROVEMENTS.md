...
## Round 5 — Requested feature batch: implemented vs. deliberately deferred

A large 22-item feature list was requested. Rather than rushing all of
them (which would have meant more unverified code exactly when
verification was already flagged as the real gap), this round triaged:
self-contained, low-risk items were implemented properly; full subsystems
were deferred with a stated reason instead of stubbed out badly.

### Implemented

- **Breadcrumbs** \u2014 the component already existed but was never
  rendered anywhere. Wired globally into `App.jsx` (auto-hides on the
  homepage, which the component already handled).
- **Honeypot spam protection** \u2014 an off-screen `website` field (not
  `display:none`, since some bots specifically skip hidden fields) that
  real users never fill. Server validates it via the existing Joi schema
  and silently fakes a success response if it's filled, giving bots no
  signal to adapt against. Layered on top of the existing rate limiting
  (`contactLimiter`), not a replacement for it.
- **Google Maps embed** on the Contact page \u2014 a plain iframe embed of
  the real head office address, no API key required.
- **Pagination** \u2014 new reusable `Pagination.jsx` component, wired into
  the Blog listing (9 posts/page).
- **Service category filter pages** \u2014 `/services?category=esg` etc.,
  each category gets a real shareable URL rather than only an in-page
  scroll position.
- **Related Services** \u2014 already existed on `ServiceDetail.jsx` from the
  routing-completeness round; confirmed working, not rebuilt.
- **Print styles** \u2014 a `@media print` block that strips nav/footer/
  toasts/buttons and resets to black-on-white, so blog posts and the
  contact page print as clean documents.
- **Reading progress bar** on blog posts \u2014 tracks actual scroll
  percentage through the document, fixed under the navbar.
- **Last-updated timestamps** \u2014 blog posts now show an "Updated" date
  alongside "Published" when the two differ.
- **Real Open Graph image** \u2014 generated a real 1200\u00d7630 PNG using the
  actual design system (Ink background, Brass accent, the Tick-Rule and
  Stamp Mark signature elements) rather than a placeholder \u2014
  `client/public/og-image.png`.
- **PWA icons completed** \u2014 regenerated all icons with the correct final
  brand palette (earlier placeholders used a slightly different navy),
  added a dedicated 180\u00d7180 apple-touch-icon and maskable 192/512
  variants with proper safe-zone padding, updated `manifest.json` with
  `purpose: "maskable"` entries.
- **ARIA live regions** \u2014 audited; toast notifications already carry
  `role="alert"` via react-toastify, newsletter popup already has
  `role="dialog"`/`aria-modal` from an earlier round \u2014 no gaps found
  requiring new work here.

### Deliberately deferred (with reasons, not silently dropped)

- **Full-text search backend** \u2014 needs a real MongoDB text index design
  and search-relevance decisions, not a quick add
- **Dark mode toggle** \u2014 touches every component's color usage; doing it
  properly needs its own pass, not a rushed toggle bolted onto light-mode
  hardcoded values
- **Service worker cache-strategy upgrade** \u2014 risk of silently breaking
  the existing PWA offline behavior if rushed
- **Event RSVP system** \u2014 a full feature: new data model, admin
  management UI, confirmation email flow
- **Newsletter archive** \u2014 new page + backend query, not yet scoped
- **Job application tracking** \u2014 new model, admin UI, status workflow
- **Compare Services** \u2014 a genuine new UI pattern needing real design
  work, not a quick table
- **Image optimization pipeline (WebP/AVIF, blur-up)** \u2014 requires
  actually re-hosting and processing real image assets, which needs a
  real asset pipeline decision (Cloudinary is already a dependency \u2014
  this is the natural next step, but wiring it project-wide is its own task)
- **Replacing Unsplash placeholders with real IPMC photos** \u2014 same
  structural limitation as team photos: no real project photography
  available to this sandbox
- **Skeleton screens** \u2014 would touch many components; noted for a
  dedicated pass rather than partial coverage

Full repo re-verified after this round: 78 client/admin files + all
server files, zero syntax failures.

---

## Round 6 — Team photos wired to real IPMC CDN, skeleton screens, error monitoring scaffold

### 21. Team photos: real IPMC photos, no longer Unsplash

Re-tested the binary-download limitation directly this round: fetched
one of the exact image URLs and got `"Image content is not supported"`
back \u2014 this confirms it's a **tool capability limit**, not a
permissions issue that a retry could work around. The fetch tool in this
sandbox can extract text/HTML; it cannot return binary image bytes, from
any source, full stop.

What *was* achievable: `client/src/data/team.js` now points every team
member's `image` field at their real photo, hotlinked directly from
IPMC's own WordPress/Jetpack CDN (re-verified against a fresh fetch of
ipmc-ng.com/about this round) \u2014 replacing the generic Unsplash stock
photos entirely. Real people instead of stock photography, even though
the files aren't self-hosted.

Also added `initialsAvatar()` \u2014 if IPMC's CDN is ever briefly
unreachable, `TeamSection.jsx` and `Team.jsx` now fall back to a clean
generated initials avatar via `onError`, instead of a broken-image icon.
`loading="lazy"` added to both as well.

**To self-host these instead of hotlinking**: run
`scripts/ipmc-team-images/download-team-images.js` anywhere with normal
internet access (unchanged from earlier rounds), then point `image` in
`team.js` at the downloaded files in a local `/images` folder.

### 22. Skeleton screens

Wired the existing `SkeletonCard` component (it existed but was never
used) into the loading states for `Blog.jsx`, `Services.jsx`, and
`Team.jsx` \u2014 replacing a centered spinner with content-shaped
placeholders matching each page's actual grid layout.

**Not done**: `ServiceDetail.jsx` and `BlogPost.jsx` still use a spinner
for their single-item loading state, and every admin manager page
(Blog/Service/Team managers, Messages) still uses `LoadingSpinner`
rather than skeletons.

### 23. Error monitoring scaffold

Added `server/utils/errorReporter.js` \u2014 a `captureError()` seam wired
into both global process handlers (`unhandledRejection`/
`uncaughtException`) and the Express error-handling middleware (5xx-class
errors only \u2014 expected 4xx validation/auth errors aren't treated as
incidents). Without `SENTRY_DSN` set, it logs locally via the existing
structured logger and does nothing else \u2014 same "safe placeholder"
pattern as the Resend integration. Enabling real Sentry from here is a
one-file change (install `@sentry/node`, set the env var, uncomment two
lines) rather than an app-wide refactor.

**Not done**: no client-side (React) error reporting hook added \u2014 only
the backend. No actual Sentry account/DSN configured (that's a decision
only you can make).

### Still open after this round

Unchanged from Round 5's deferred list except where noted above: full-
text search, dark mode toggle, service worker cache-strategy upgrade,
Event RSVP, newsletter archive, job application tracking, Compare
Services, a real image optimization pipeline (WebP/AVIF/blur-up),
accessibility audit, load/performance testing, legal review, and running
this codebase for real anywhere outside this sandbox.
