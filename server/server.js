...
const { captureError } = require('./utils/errorReporter');
...
// (and, once SENTRY_DSN is set, an external monitoring service too)
// instead of vanishing.
process.on('unhandledRejection', (reason) => {
  captureError(reason instanceof Error ? reason : new Error(String(reason)), { type: 'unhandled_rejection' });
});
process.on('uncaughtException', (err) => {
  captureError(err, { type: 'uncaught_exception' });
